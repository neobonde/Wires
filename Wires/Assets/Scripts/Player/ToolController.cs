...
public class ToolController : MonoBehaviour
{
    ...
    public void CreateItem()
    {
        ...
        Vector2 spawnLocation = player.position + new Vector3(-2,2,0);
        switch (SelectedItem)
        {
            default:
            case ItemType.GATE:
                Instantiate(GatePrefab,spawnLocation,Quaternion.identity);
                break;
            case ItemType.PLATE:
                Instantiate(PlatePrefab,spawnLocation,Quaternion.identity);
                break;
            case ItemType.LED:
                Instantiate(LedPrefab,spawnLocation,Quaternion.identity);
                break;
            case ItemType.LEVER:
                Instantiate(LeverPrefab,spawnLocation,Quaternion.identity);
                break;
            case ItemType.WEIGHT:
                Instantiate(WeightPrefab,spawnLocation,Quaternion.identity);
                break;
            case ItemType.SPLITTER:
                Instantiate(SplitterPrefab,spawnLocation,Quaternion.identity);
                break;
        }
    }
